...
export class Body extends React.Component<Props, State> {
    ...
    framesForPlayer(allFrames:any[], contestantIndex:number, maxFrames:number) {
      ...
      renderFrameCell(rolls:any[], index:number) {
        ...
        let scores = rolls.map((roll, i) => (
          <span key={i} className="roll-score">{roll}</span>
        ));
        return (
          <td key={index}>
            {scores}
      ...
      render(): React.ReactNode{
        return (
            <tbody>
              {this.props.contestants.map(this.renderRow)}
            </tbody>
          );
      }
}
